...
module.exports = function(callback) {
	...
	if (this.currentAction === "query" || this.currentAction === "scan") {
		DynamoDB[this.currentAction](this.params, function(error, response) {
			if (!error) {
				response.Items = response.Items.map(AWStoJSON);
			}
			callback(error, response);
		});
	}
	...
	if (this.currentAction === "updateItem" || this.currentAction === "deleteItem" || this.currentAction === "createTable" || this.currentAction === "updateTable") {
		DynamoDB[this.currentAction](this.params, function(error, response) {
			callback(error, response);
		});
	}
